...
so the placeholder tile is not a manifest problem to fix.

`https://geminicli.com/extensions.json` has no icon, logo, or Open Graph
field. The only image it carries is `avatarUrl`, and that is the **GitHub
owner avatar** (`https://avatars.githubusercontent.com/u/23102767?v=4` for
`TonyXhufi`). Repo Social preview, `assets/icon-512.png`, and the README
embed are all ignored. Diagnosed 2 Sep 2026 against a live gallery row that
already had a correct Social preview and still rendered the tan GitHub
identicon.

The one step that changes the tile:

> github.com/settings/profile → upload `icon-512.png`

The gallery hotlinks that avatar URL, so the tile updates when GitHub's CDN
refreshes — no recrawl, no manifest bump. Transferring the repo to an org
whose avatar is the WEM mark is the same mechanism with a different owner.

## The files

| File | Size | Use |
| --- | --- | --- |
| `icon-512.png` | 512×512 | **Gemini gallery tile** — upload as the `TonyXhufi` GitHub profile photo. Also MCP directories, README badges, the press kit. |
| `icon-1024.png` | 1024×1024 | Same mark where a larger square is required. |
| `social-preview-1280x640.png` | 1280×640 | GitHub **Settings → General → Social preview** on `TonyXhufi/wem-gemini-extension`. Link-unfurl / OG image only. **Not** the geminicli.com tile. |
